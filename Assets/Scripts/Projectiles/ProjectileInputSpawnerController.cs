...
public class ProjectileInputSpawnerController : MonoBehaviour, IInteractable
{
    ...
    public void OnDragEnd(Vector2 pos)
    {
        ...
        var launchForce = DragIndicator.GetLaunchVector() * ForceMultiplier;
        var launchPosition = DragIndicator.GetLaunchPosition();

        DragIndicator.EndDrag();

        var spawnPosition = launchPosition.ClosestPointInCircle(transform.position, Collider.radius);
        ProjectileSpawner.SpawnProjectile(PlayerManager.Instance.SelectedProjectileData, spawnPosition, launchForce);
    }
